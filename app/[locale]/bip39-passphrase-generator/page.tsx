...
export default function BIP39PassphraseGenerator() {
  ...
  // 从熵生成助记词
  const generateMnemonicFromEntropy = useCallback((entropyValue: string) => {
    try {
      if (!entropyValue) {
        setMnemonic("");
        return;
  ...
  // 从助记词生成熵
  const generateEntropyFromMnemonic = useCallback((mnemonicValue: string) => {
    try {
  ...
  // 处理熵值变化
  const handleEntropyChange = useCallback((value: string) => {
    setEntropy(value);
  ...
  // 验证状态
  const isEntropyValid = entropy ? /^[0-9a-fA-F]+$/.test(entropy) && entropy.length % 2 === 0 : true;
      ...
      <Card className="w-full max-w-4xl mx-auto">
        <CardContent className="space-y-6">
          ...
          <div className="space-y-2">
            ...
            />
            {!isEntropyValid && (
              <p className="text-sm text-red-500">{t("invalidEntropy")}</p>
            )}
          </div>
          ...
          <div className="space-y-2">
            ...
            />
            {!isMnemonicValid && (
              <p className="text-sm text-red-500">{t("invalidMnemonic")}</p>
            )}
          </div>
        </CardContent>
      </Card>
